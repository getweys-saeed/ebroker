...
class SettingController extends Controller
{
    ...
    public function app_settings(Request $request)
    {
        if (!has_permissions('update', 'app_setting')) {
            ...
            if ($request->hasFile('splash_logo')) {
                ...
                // Get Data from Settings table
                $splashLogoDatabaseData = system_setting('splash_logo');
                $databaseData = !empty($splashLogoDatabaseData) ? $splashLogoDatabaseData : null;

                $input['splash_logo'] = handleFileUpload($request, 'splash_logo', $destinationPath, $file->getClientOriginalName(), $databaseData);
            }
            if ($request->hasFile('app_home_screen') && $request->file('app_home_screen')->isValid()) {
                ...
                $input['app_home_screen'] = handleFileUpload($request, 'app_home_screen', $destinationPath, $file->getClientOriginalName(), $databaseData);
            }
            if ($request->hasFile('placeholder_logo') && $request->file('placeholder_logo')->isValid()) {
                ...
                $input['placeholder_logo'] = handleFileUpload($request, 'placeholder_logo', $destinationPath, $file->getClientOriginalName(), $databaseData);
            }
    ...
    public function notificationSettingStore(Request $request){
        if (!has_permissions('update', 'notification')) {
            return redirect()->back()->with('error', PERMISSION_ERROR_MSG);
        } else {
