        ...
        */
        if (env('DEMO_MODE') && !$request->isMethod('get') && Auth::check()) {
            if(in_array(Auth::user()->email, $excludeEmails)){
                return $next($request);
            }else if(Auth::user()->firebase_id != 'sg6XM3VTneYnBI5xeJkZ1Dxti3f1' && Auth::user()->mobile != '919764318246' && Auth::user()->email != 'wrteamdemo@gmail.com' && Auth::user()->email != 'admin@gmail.com'){
                return $next($request);
            }else{
